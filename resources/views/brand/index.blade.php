    ...
    <div class="card shadow">
        <div class="card-body">
            <div class="toolbar row mb-3">
                <div class="col">
                    <form class="form-inline" action="{{route('item.brand.all',['type'=>$type])}}">
                ...
                <table class="table table-hover">
                    ...
                    @foreach($brands as $brand)
                        <tr>
                            ...
                            <td>{{$brand->name}}</td>
                           <td> {{date_format($brand->created_at,'m/d/y h:iA')}}</td>
                            <td>
                                <a data-toggle="modal" data-target="#repairModal" data-id="{{ $brand->id }}"
                                   data-name="{{ $brand->name }}" class="btn btn-primary edit"><i
                                            class="fa-solid fa-pen"></i></a>
                                <a data-toggle="modal" data-target="#deleteModel" data-id="{{ $brand->id }}"
                                   data-name="{{ $brand->name }}" class="btn btn-danger delete"><i
...
@push('script')
    <script>
        (function ($) {
            ...
            var saveAction = `{{ route('item.brand.save',['type'=>$type]) }}`;
            var deleteAction = `{{ route('item.brand.delete') }}`;
